...
def build_provider_reliability_ranking(health, daily_metrics=None):
    health = health.copy()
    health["provider"] = health["provider"].map(normalize_provider_name)
    ...
    if daily_metrics is not None and not daily_metrics.empty:
        ...
        metrics["provider"] = metrics["provider"].map(normalize_provider_name)
        if "date" in metrics.columns:
            metrics = metrics.drop_duplicates(subset=["date", "provider"], keep="last")
        ...
        if "score" in metrics.columns:
            metrics["score"] = pd.to_numeric(metrics["score"], errors="coerce").fillna(0)
        else:
            metrics["score"] = 0
        ...
        for provider, group in grouped:
            ...
            avg_rank_score = group["score"].mean()
    ...
    for _, row in health.iterrows():
        ...
        rank_score = clamp(float(metrics.get("avg_rank_score", 0)))

        reliability_score = round(clamp(
            (float(row["health_score"]) * 0.25)
        ...
        enriched.update({
            ...
            "provider_rank_score": round(rank_score, 2),
