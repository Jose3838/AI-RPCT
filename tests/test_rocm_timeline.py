...
CSV_PATH = ROOT / "data" / "rocm_timeline.csv"
WAREHOUSE_PATH = ROOT / "warehouse" / "historical" / "rocm" / "rocm_timeline.csv"
...
def test_builder_runs():
    result = subprocess.run(
        [sys.executable, str(ROOT / "analytics" / "build_rocm_timeline.py")],
        cwd=ROOT,
        text=True,
        capture_output=True,
        check=True,
...
def test_rocm_timeline_files_exist():
    assert CSV_PATH.exists()
    assert WAREHOUSE_PATH.exists()


def test_rocm_timeline_not_empty():
...
def test_software_stack_is_rocm():
    assert {row["software_stack"] for row in load_rows(CSV_PATH)} == {"ROCm"}


def test_vendor_is_amd():
    assert {row["vendor"] for row in load_rows(CSV_PATH)} == {"AMD"}
