...
CSV_PATH = ROOT / "data" / "synapseai_timeline.csv"
WAREHOUSE_PATH = ROOT / "warehouse" / "historical" / "synapseai" / "synapseai_timeline.csv"
...
def test_builder_runs():
    result = subprocess.run(
        [sys.executable, str(ROOT / "analytics" / "build_synapseai_timeline.py")],
        cwd=ROOT,
        text=True,
        capture_output=True,
        check=True,
...
def test_synapseai_timeline_files_exist():
    assert CSV_PATH.exists()
    assert WAREHOUSE_PATH.exists()


def test_synapseai_timeline_not_empty():
...
def test_software_stack_is_synapseai():
    assert {row["software_stack"] for row in load_rows(CSV_PATH)} == {"SynapseAI"}
